...
def initialize_db(db_path: str) -> None:
    ...
    try:
        ...
        connection.execute("INSERT INTO admin_config (id, api_key) VALUES (1, ?)", ("test-key",))
        connection.execute(
            """
...
def set_admin_api_key(db_path: str, api_key: str) -> None:
    connection = sqlite3.connect(db_path)
    try:
        connection.execute("UPDATE admin_config SET api_key = ? WHERE id = 1", (api_key,))
        connection.commit()
    finally:
        connection.close()
...
class AdapterApiTests(unittest.TestCase):
    ...
    def test_create_returns_gateway_timeout_when_task_id_is_delayed(self):
        ...
        main_service_client = build_main_service_client(
            done_event=done_event,
            chunks=[
                (0.2, b'data: {"choices":[{"delta":{"output":[{"task_id":"late_task"}]}}]}\n\n'),
                (0.01, b"data: [DONE]\n\n"),
            ],
        )
    ...
    def test_get_generation_maps_statuses_and_fields(self):
        ...
        created_at_expected = int(datetime.fromisoformat("2026-03-24 12:00:00+00:00").timestamp())
        completed_at_expected = int(datetime.fromisoformat("2026-03-24 12:10:00+08:00").timestamp())
    ...
    def test_startup_fails_when_admin_config_api_key_does_not_match(self):
        set_admin_api_key(self.db_path, "db-key")

        with self.assertRaisesRegex(RuntimeError, "does not match admin_config.api_key"):
            with TestClient(
                create_app(
                    settings=make_settings(self.db_path, POLO_ADAPTER_API_KEY="adapter-key"),
                    main_service_client=NeverCalledMainServiceClient(),
