...
async def create_video(
    ...
    request.app.state.background_streams.start(
        # Keep draining immediately so timeout responses do not abandon the upstream stream.
        name=f"drain-{request.state.request_id}",
        coro=session.drain(),
    )
    task_id = await session.wait_for_task_id()
...
async def get_video_generation(
    ...
    return VideoGenerationResponse(
        ...
        created_at=parse_created_timestamp(record.created_at),
        completed_at=parse_completed_timestamp(record.completed_at, request.app.state.settings.main_local_tz),
    )
