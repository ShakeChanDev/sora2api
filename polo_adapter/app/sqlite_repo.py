    ...
    async def validate_shared_api_key(self, expected_api_key: str) -> None:
        ...
        try:
            try:
                ...
                raise RuntimeError(
                    f"Unable to validate POLO_ADAPTER_API_KEY against admin_config.api_key: {exc}"
                ) from exc
        finally:
            await db.close()
        ...
        actual_api_key = str(row["api_key"] or "")
        if actual_api_key != expected_api_key:
            raise RuntimeError("POLO_ADAPTER_API_KEY does not match admin_config.api_key")
